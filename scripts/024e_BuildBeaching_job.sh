#!/bin/bash
#SBATCH --job-name=024e_BuildBeaching
# One cell per (year, month) = |YEARS| x 12 = 48 cells, but each cell is now a
# seconds-per-sidecar numpy pass (~2.5 s x 73 zarrs for a whole month), so a
# small allocation drains the grid in minutes. Raise --ntasks on the command
# line when sweeping several members in one job.
#SBATCH --ntasks=12
...
# compact BeachingForcing sidecar (~30 MB per zarr) and the key parquet, so
# they are CPU-bound rather than GPFS-bound and packing them tight costs
# nothing.

# Beaching reducer: reads the BeachingForcing sidecar written by
# 024d_BuildBeachingForcing plus the 024a key, writes one beaching parquet per
# (regime, release_year, release_month, rate-model member). No trajectory
# zarrs, no raw Stokes, no raster.
#
# Usage: sbatch scripts/024e_BuildBeaching_job.sh [regime] [hex_radius] [stagger_max_s]
#
...
MAX_FLOAT_DAYS="${MAX_FLOAT_DAYS:-60}"
...
for year in "${YEARS[@]}"; do
    ...
    for attempt in 1 2 3; do
        srun --ntasks=1 --cpus-per-task=${SLURM_CPUS_PER_TASK} --exact \
            ...
            notebooks/024e_BuildBeaching.ipynb \
            notebooks_executed/Visualisations/024e_BuildBeaching_${regime}_${year}${ms}_${member}_r${hex_radius}m.ipynb \
            ...
            -p delta ${DELTA} \
            -p band_m ${BAND_M} \
            -p max_float_days ${MAX_FLOAT_DAYS} \
            -k python
        rc=$?
        [ ${rc} -eq 0 ] && break

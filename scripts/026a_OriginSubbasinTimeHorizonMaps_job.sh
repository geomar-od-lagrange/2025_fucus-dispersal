...
# time-horizon figure per origin (HELCOM) subbasin. Submit once per
# (regime, hex_radius), e.g.:
#   sbatch scripts/026a_OriginSubbasinTimeHorizonMaps_job.sh surface 6000
#   sbatch scripts/026a_OriginSubbasinTimeHorizonMaps_job.sh bottom 6000
...
# origin_subbasins falls back to the notebook default (all named subbasins
# that seed releases).
...
pixi run papermill --cwd notebooks/ \
    ...
    notebooks_executed/Visualisations/026a_OriginSubbasinTimeHorizonMaps_${regime}_r${hex_radius}m.ipynb \

...
- **Done:** beaching design (§5) — see [beaching.md](beaching.md).
...
## 5. Beaching and remobilisation — ✅ planned, see [beaching.md](beaching.md)

**Planning complete.** The post-simulation beaching model is fully
specified in [beaching.md](beaching.md): a first-stranding endpoint scored
from a float/viability gate, a per-position distance to the fine
coastline, a wall/flat retention weight, and **onshore cross-shore wave
Stokes** as the driver — the near-shore transport the runs deliberately
mask at blocked BSH faces (fetch-aware and consistent with the
`surface_stokes` runs; the earlier "wind" driver was superseded). The
design folds in a short literature basis with DOIs.

Open work is **implementation**: `024d_BuildBeaching` (heavy pass over the
zarrs + raw CMEMS Stokes) and `029_BeachingMaps` (parquet-only consumer),
plus the parameter decisions listed in the design note.
